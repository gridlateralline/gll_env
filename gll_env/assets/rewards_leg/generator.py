...
def generate_and_save_solarquartier_asset() -> None:
    ...
    Unlike the constructed tariff below, this one is not derived from a
...
def generate_and_save_fair_leg_asset() -> None:
    ...
    The VNB rates are the members' fallback and are left untouched: the
    published EEA feed-in tariff for injection, and ewz.natur energy + grid
    usage + public duties for consumption. Trading inside the LEG waives
    :data:`GRID_REBATE_SHARE` of the grid usage fee, and that saving is shared
    evenly — the injector is paid half of it on top of the feed-in tariff, the
    consumer keeps the other half off their bill. Symmetric by construction,
    hence "fair": neither side captures the community's saving.

    Because only the *matched* energy is settled at LEG rates (see
    :class:`~gll_env.rewards.leg.LegSettlementReward`), the rebate accrues
    exactly on the energy actually traded inside the community, and nowhere
    else.
    """
    vnb_consumption_nt = NATUR_ENERGY_NT + GRID_USAGE_NT + PUBLIC_DUTIES
    vnb_consumption_ht = NATUR_ENERGY_HT + GRID_USAGE_HT + PUBLIC_DUTIES

    # Grid usage is negative, so negate to get the (positive) saving, then halve it.
    rebate_half_nt = -GRID_USAGE_NT * GRID_REBATE_SHARE / 2.0
    rebate_half_ht = -GRID_USAGE_HT * GRID_REBATE_SHARE / 2.0

    payments = _payments(
        leg_injection=(EEA_INJECTION_NT + rebate_half_nt, EEA_INJECTION_HT + rebate_half_ht),
        vnb_injection=(EEA_INJECTION_NT, EEA_INJECTION_HT),
        leg_consumption=(
            vnb_consumption_nt + rebate_half_nt,
            vnb_consumption_ht + rebate_half_ht,
        ),
        vnb_consumption=(vnb_consumption_nt, vnb_consumption_ht),
    )
